...
def detect_objects(img):
    ...
    for i in indices:
        ...
        distance=int(distance*0.025)
        #str("{:.2f} Inches".format(distance))
        cv2.rectangle(img, (x,y),(x+w,h+y), color=(0, 255, 0), thickness=2)
        #cv2.putText(img,label +" " + confidence,(x,y+20),font,2,(34,139,34),2)
        cv2.putText(img,classNames[classIds[i][0]-1].upper() +" "+str(distance)+"m",(box[0]+10,box[1]+30),
...
while True:
    ...
    detect=detect_objects(heading_image)
    #plt.imshow(heading_image)
    #plt.show()
    cv2.imshow("output",detect)
